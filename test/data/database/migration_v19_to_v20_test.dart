...
void main() {
  group('Migration v19 -> current', () {
    test(
      'migrated database preserves semantic modifier rows and adds source_group_id',
      () async {
        final AppDatabase db = _createV19ThenMigrateToCurrent();
        ...
        expect(columns, contains('source_group_id'));
        ...
        expect(row.read<String>('action'), 'choice');
        expect(row.read<String>('item_name'), 'Tea');
        expect(row.read<int>('item_product_id'), 3);
        expect(row.read<String>('charge_reason'), 'included_choice');
        expect(row.read<int>('sort_key'), 11);
    ...
    test(
      ...
      () async {
        ...
        final AppDatabase migratedDb = _createV19ThenMigrateToCurrent();
    ...
    test('migrated schema rejects invalid source_group_id values', () async {
      final AppDatabase db = _createV19ThenMigrateToCurrent();
      addTearDown(db.close);
...
AppDatabase _createV19ThenMigrateToCurrent() {
  final QueryExecutor rawDb = NativeDatabase.memory(
    setup: (database) {
      ...
      ''');
      database.execute(
        "INSERT INTO transaction_lines (id, uuid, transaction_id, product_id, product_name, unit_price_minor, quantity, line_total_minor, pricing_mode, removal_discount_total_minor) VALUES (1, 'line-1', 1, 1, 'Set 4', 400, 1, 400, 'set', 0);",
      );
      database.execute(
        "INSERT INTO order_modifiers (id, uuid, transaction_line_id, action, item_name, quantity, item_product_id, extra_price_minor, charge_reason, unit_price_minor, price_effect_minor, sort_key) VALUES (1, 'modifier-1', 1, 'choice', 'Tea', 1, 3, 0, 'included_choice', 0, 0, 11);",
      );
      database.execute(
