...
void main() {
  group('Meal customization order integration', () {
    test(
      'same customization merges into one grouped line with quantity-aware totals',
      () async {
        ...
        final List<QueryRow> lineRows = await db
            .customSelect(
              '''
        ...
        final Transaction? refreshedOrder = await service.getOrderById(
          order.id,
        );
    ...
    test('different swap customizations do not merge', () async {
      ...
      expect(
        lineRows.map((QueryRow row) => row.read<int>('quantity')),
        everyElement(1),
      );
      expect(
        lineRows
            .map((QueryRow row) => row.read<int>('line_total_minor'))
            .toList(),
        <int>[1000, 1050],
      );
    });
    ...
    test(
      'same product default and customized states stay on separate lines',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
    ...
    test(
      'profile-bound standard product persists resolved semantic snapshot',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
    ...
    test(
      'breakfast semantic product assigned a meal profile throws a controlled runtime configuration error',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
        ...
        await expectLater(
          ...
          throwsA(
            isA<MealCustomizationRuntimeConfigurationException>().having(
              (MealCustomizationRuntimeConfigurationException error) =>
                  error.message,
              'message',
              contains(
                'Product "Set Breakfast" cannot use meal-adjustment profile "Lunch meal profile" because it is configured as a breakfast semantic root product.',
              ),
            ),
          ),
        );
    ...
    test(
      'grouped line decrement contract preserves snapshot until final delete',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
        ...
        final TransactionRepository transactionRepository =
            TransactionRepository(db);
        final MealCustomizationPosService posService =
            MealCustomizationPosService(
        ...
        expect(
          rehydrated.editorState.componentSelections,
    ...
    test(
      ...
      () async {
        ...
        final TransactionLine updatedLine = await service
        ...
        expect(snapshot.customizationKey, snapshot.snapshot.stableIdentityKey);
        expect(
        ...
        expect(snapshot.snapshot.toEditorState().removedComponentKeys, <String>[
          'side',
        ]);
      },
    );
    ...
    test(
      'exact remove combo persists and rehydrates both remove states',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
        ...
        expect(line.lineTotalMinor, 750);
        expect(snapshotRecord.snapshot.appliedRuleIds, hasLength(1));
        expect(
          snapshotRecord.snapshot.triggeredDiscounts.single.chargeReason,
          MealCustomizationChargeReason.comboDiscount,
        );
        expect(
          snapshotRecord.snapshot.appliedRules.single.ruleType,
    ...
    test(
      'snapshot-backed line supports keep to remove and remove to keep edits',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
        ...
        final Transaction beforeRemove = (await service.getOrderById(
          order.id,
        ))!;

        final TransactionLine removedLine = await service
            .editMealCustomizationLine(
              transactionLineId: line.id,
              request: fixture.buildRequest(removeSide: true),
              expectedTransactionUpdatedAt: beforeRemove.updatedAt,
            );
        final MealCustomizationPersistedSnapshotRecord removedSnapshot =
            (await transactionRepository.getMealCustomizationSnapshotByLine(
              removedLine.id,
            ))!;

        expect(removedLine.lineTotalMinor, 950);
        expect(
          removedSnapshot.snapshot.toEditorState().removedComponentKeys,
          <String>['side'],
        );

        final Transaction beforeKeep = (await service.getOrderById(order.id))!;
        final TransactionLine keptLine = await service
            .editMealCustomizationLine(
              transactionLineId: removedLine.id,
              request: fixture.buildRequest(),
              expectedTransactionUpdatedAt: beforeKeep.updatedAt,
            );
        final MealCustomizationPersistedSnapshotRecord keptSnapshot =
            (await transactionRepository.getMealCustomizationSnapshotByLine(
              keptLine.id,
            ))!;

        expect(keptLine.lineTotalMinor, 1000);
        expect(
          keptSnapshot.snapshot.toEditorState().componentSelections,
          isEmpty,
        );
      },
    );
    ...
    test(
      'editing into an existing semantic identity merges grouped meal lines',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
        ...
        final TransactionLine mergedLine = await service
            .editMealCustomizationLine(
        ...
        expect(
          await transactionRepository.getLineById(targetLine.id),
          isNotNull,
        );
        expect(
          (await transactionRepository.getLineById(targetLine.id))!.quantity,
          2,
        );
        expect(
          (await transactionRepository.getLineById(
            targetLine.id,
          ))!.lineTotalMinor,
          2100,
        );
    ...
    test(
      'legacy standard meal lines without snapshot persistence are blocked',
      () async {
        final db = createTestDatabase();
        addTearDown(db.close);
