...
void main() {
  group('Breakfast runtime integration', () {
    test('choice overflow persists semantic rows and recalculates totals', () async {
      final db = createTestDatabase();
      addTearDown(db.close);
      final _BreakfastFixture fixture = await _seedBreakfastFixture(db);

      final order = await fixture.service.createOrder(currentUser: fixture.cashier);
      ...
      final modifiers = await fixture.service.getLineModifiers(updatedLine.id);
    ...
    test('rebuild clears prior classified rows instead of patching incrementally', () async {
      final db = createTestDatabase();
      addTearDown(db.close);
      final _BreakfastFixture fixture = await _seedBreakfastFixture(db);
      ...
      final TransactionLine afterClear = await fixture.service.editBreakfastLine(
        transactionLineId: line.id,
        edit: BreakfastLineEdit.clearGroup(groupId: fixture.toastBreadGroupId),
      );
    ...
    test('third replacement persists as paid_swap and totals use price_effect_minor', () async {
      final db = createTestDatabase();
      addTearDown(db.close);
      final _BreakfastFixture fixture = await _seedBreakfastFixture(db);
      ...
      final TransactionLine updatedLine = await fixture.service.editBreakfastLine(
        transactionLineId: line.id,
        edit: BreakfastLineEdit.setAddedQuantity(
    ...
    test('choice product never becomes swap', () async {
      final db = createTestDatabase();
      addTearDown(db.close);
      final _BreakfastFixture fixture = await _seedBreakfastFixture(db);
      ...
      final modifiers = await fixture.service.getLineModifiers(updatedLine.id);
      final OrderModifier teaRow = modifiers.singleWhere(
        (OrderModifier modifier) => modifier.itemProductId == fixture.teaProductId,
      );
      expect(teaRow.chargeReason, ModifierChargeReason.extraAdd);
      expect(teaRow.priceEffectMinor, 150);
      expect(
        modifiers.where(
          (OrderModifier modifier) =>
              modifier.chargeReason == ModifierChargeReason.freeSwap ||
    ...
    test('paid and cancelled breakfast lines are not editable', () async {
      ...
      final paidOrder = await fixture.service.createOrder(currentUser: fixture.cashier);
    ...
    test('different breakfast configs split into separate lines', () async {
      ...
      final TransactionLine customizedLine = await fixture.service.editBreakfastLine(
        transactionLineId: line.id,
        edit: BreakfastLineEdit.chooseGroup(
      ...
      expect(lines.map((TransactionLine item) => item.quantity), everyElement(1));
    });
  });
}
...
Future<_BreakfastFixture> _seedBreakfastFixture(app_db.AppDatabase db) async {
  ...
  final int breakfastCategoryId = await insertCategory(db, name: 'Set Breakfast');
  final int hotDrinkCategoryId = await insertCategory(db, name: 'Hot Drink');
  final int extrasCategoryId = await insertCategory(db, name: 'Breakfast Extras');
  ...
  final int hotDrinkGroupId = await db.into(db.modifierGroups).insert(
  ...
  final int toastBreadGroupId = await db.into(db.modifierGroups).insert(
    app_db.ModifierGroupsCompanion.insert(
      productId: set4ProductId,
      name: 'Toast or Bread',
      minSelect: const Value<int>(0),
